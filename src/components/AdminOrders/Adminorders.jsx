        ...
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            try {
              ...
              // Convert any stringified products again (if needed)
              products = products.map((prod) =>
                typeof prod === "string" ? JSON.parse(prod) : prod
              );
            } catch (err) {
              console.error("Error processing productIds:", err);
            }
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm">
                      <span className="font-medium">Ship to:</span> {shipTo}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {products.map((product, index) => {
                      const imageUrl = product.image
                        ? storage.getFileView(BUCKET_ID, product.image).toString()
                        : null;
                      ...
                      return (
                        <div key={index} className="flex items-start gap-4">
                          {imageUrl ? (
                            ...
                            <div className="w-20 h-20 flex items-center justify-center bg-gray-100 text-sm text-gray-500 rounded-lg">
                              No Image
                            </div>
                          )}
                          <div>
